const getDataOfAllCountries = async () => {
  ...
  let countriesAsObjects = [];
  for (const code of codesArray) {
    const response = await fetch(
      `https://restcountries.eu/rest/v2/alpha/${code}?fields=name;capital;translations;alpha2Code`
    );
    ...
    if (countryData) {
      ...
      countriesAsObjects.push(object);
    }
  }
  return countriesAsObjects;
};
